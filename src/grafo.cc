 ...
 */
const std::vector<std::vector<double>>& Grafo::GetMatrizCoste() const {
    ...
    for (const auto& fila : matriz_adyacencia_) {
        for (double costo : fila) {
            if (costo == -1) {
                ...
                std::cout << std::setw(7) << std::left << costo << " ";
            }
        }
        std::cout << std::endl;
    }
...
void Grafo::InformacionConexiones(std::ofstream& fichero_salida) const {
    ...
    int contador_aristas = 1;   // Contador para numerar las aristas
    for (int i = 0; i < num_vertices_; ++i) {   // Recorremos filas
        for (int j = i + 1; j < num_vertices_; ++j) {   // Recorremos columnas (solo mitad superior)
            // Si hay conexión y el coste no es -1
            if (matriz_adyacencia_[i][j] != -1) {
                // Imprimimos la arista con su número, nodos y coste
                fichero_salida << contador_aristas << ".- Conexión (" << i + 1 << ", " << j + 1 << ") y (" << j + 1 << ", " << i + 1 << ") = " << matriz_adyacencia_[i][j] << "\n";
                contador_aristas++;
            }
        }
    }
    ...
    for (int i = 0; i < num_vertices_; ++i) { // Recorremos cada nodo
        // Recogemos los vecinos, excluyendo al propio nodo (i == j)
        std::vector<int> hijos; // Vector para almacenar los hijos
        for (int j = 0; j < num_vertices_; ++j) {   // Recorremos todas las columnas
        ...
        if (!hijos.empty()) {   // Si tiene hijos, los listamos
